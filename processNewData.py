...
from patti_common import (
    fmt_local_human,
    normalize_patti_body,
    append_soft_schedule_sentence,
    rewrite_sched_cta_for_booked,
)
...
def _already_sent_first_touch_recently(
    *, customer_email: str, subscription_id: str, current_opp_id: str, lookback_hours: int = 72
) -> bool:
...
def is_tustin_kia_rooftop(rooftop_name: str) -> bool:
    return (rooftop_name or "").strip().lower() == "patterson tustin kia" or (
        rooftop_name or ""
    ).strip().lower() == "tustin kia"
...
def resolve_customer_email(
    opportunity: dict, *, SAFE_MODE: bool = False, test_recipient: str | None = None
) -> str | None:
    ...
    log.info(
        "EMAIL_DEBUG opp=%s customer_email=%r cust.email=%r emails_count=%d",
        opp_id,
        air_email,
        cust.get("email"),
...
""".strip()
    ...
    try:
        response = run_gpt(prompt, cn, "Tustin Kia", prevMessages=False)
...
def maybe_send_tk_day3_walkaround(
    *,
    ...
    log.info("DAY3 ROOFTOP DEBUG: opp=%s rooftop_name=%r is_tustin_kia=%s", opportunityId, rooftop_name, is_tk_rooftop)
    ...
    if not video_url:
        log.info("TK Day3 Walkaround: no video for model=%r opp=%s", vehicle_model, opportunityId)
        return False

    # Resolve customer email (like GM Day 2 - simpler call)
    to_addr = resolve_customer_email(opportunity, SAFE_MODE=SAFE_MODE, test_recipient=test_recipient)
    log.info("DAY3 EMAIL DEBUG: opp=%s resolve_customer_email returned=%r (simplified call)", opportunityId, to_addr)
    if not to_addr:
    ...
    subject = TK_DAY3_WALKAROUND_SUBJECT.format(
        vehicle_make=vehicle_make or "Kia", vehicle_model=vehicle_model or "vehicle"
    )
...
STATE_KEYS = ("mode", "last_template_day_sent", "nudge_count", "last_customer_msg_at", "last_agent_msg_at")
...
def _kbb_flags_from(opportunity_doc: dict, fresh_opp: dict | None) -> dict:
    ...
    sub = _first_present_lc(fresh_opp, "subStatus", "substatus") or _first_present_lc(
        opportunity_doc, "subStatus", "substatus"
    )
    upt = _first_present_lc(fresh_opp, "upType", "uptype") or _first_present_lc(opportunity_doc, "upType", "uptype")
    return {"source": src, "status": st, "substatus": sub, "uptype": upt}


def _is_kbb_ico(doc_flags: dict) -> bool:
    return (
        doc_flags["source"] == "kbb instant cash offer"
        and doc_flags["status"] == "active"
        and doc_flags["substatus"] == "new"
        and doc_flags["uptype"] == "campaign"
    )
...
def _is_kbb_ico_new_active(doc: dict) -> bool:
    ...
    return source in _KBB_SOURCES and status == "active" and uptype == "campaign" and substatus in {"new", "working"}
        ...
        if activityName == "read email" or activityType == 20:
            ...
            try:
                ...
                if not already_scheduled:
                    ...
                    log.info(
                        "Scheduling Intent: %s (iso=%r, conf=%.2f) opp=%s",
                        intent_action,
                        appt_iso,
                        conf,
            ...
            sub_status = ((opportunity.get("subStatus") or opportunity.get("substatus") or "")).strip().lower()

            has_booked_appt = (
                mode == "scheduled" or ("appointment" in sub_status) or bool(patti_meta.get("appt_due_utc"))
            )
...
def _derive_appointment_from_sched_activities(opportunity, tz_name="America/Los_Angeles"):
    ...
    try:
        ...
        for a in sched:
            ...
            is_appt = "appointment" in raw_name or t_str in ("2", "appointment")
            if not is_appt:
                continue
        ...
        try:
            log.warning(
                "Failed to derive appointment from scheduledActivities for %s: %s", opportunity.get("opportunityId"), e
            )
        except Exception:
            pass
        return False
...
def processHit(hit):
    ...
    subscription_id = (
        opportunity.get("_subscription_id") or fields.get("subscription_id") or fields.get("Subscription ID")
    )
    ...
    if OFFLINE_MODE:
        opp_messages = opportunity.get("completedActivitiesTesting") or opportunity.get("messages") or []
    else:
    ...
    salesperson = SALES_PERSON_MAP.get(first_name) or SALES_PERSON_MAP.get(full_name) or full_name or "our team"
    ...
    if _is_exact_kbb_ico_flags(flags, opportunity):
        ...
        try:
            ...
            mode = (state.get("mode") if isinstance(state, dict) else None) or (
                opportunity.get("_kbb_state") or {}
            ).get("mode")
            ...
            if mode == "convo":
                ...
                opportunity.setdefault("checkedDict", {})["exit_type"] = (
                    opportunity.get("checkedDict", {}).get("exit_type") or "customer_engaged"
                )
                log.info(
                    "KBB ICO: convo mode → parked followUP_date=%s opp=%s", opportunity["followUP_date"], opportunityId
                )
                ...
                if next_day is not None:
                    ...
                    log.info(
                        "KBB ICO: scheduled next followUP_date=%s opp=%s anchor_day=%s next_day=%s anchor_iso=%s",
                        opportunity["followUP_date"],
                        opportunityId,
                        anchor_day,
                        next_day,
                        anchor_iso,
    ...
    if has_booked_appt:
        log.info(
            "Opp %s has booked appointment (mode=%r, subStatus=%r); " "suppressing Patti follow-up cadence.",
            opportunityId,
            mode,
            sub_status,
        )
    ...
    from airtable_store import (
        already_contacted_airtable,
        should_suppress_all_sends_airtable,
        get_mode_airtable,
        is_customer_replied_airtable,
    )

    completedActivities = activities.get(
        "completedActivities", []
    )  # keep; you still need this for firstActivity parsing
    ...
    if not patti_already_contacted:
        ...
        if firstActivity:
            ...
            if not OFFLINE_MODE:
                ...
                firstActivityFull = {
                    ...
                    "completedDate": newest.get("completedDate") or _dt.now(_tz.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
            ...
            if not raw_inquiry:
                raw_inquiry = (
                    ((firstActivityFull or {}).get("message", {}) or {}).get("subject", "")
                    or (firstActivityFull or {}).get("notes")
                    or (firstActivity or {}).get("title")
                    or "Hi! I'm interested in this vehicle and had a few questions."
                ).strip()

            # Find or initialize conversation array
            conv = opportunity.get("messages") or opportunity.get("conversation") or opportunity.get("thread") or []
            if not isinstance(conv, list):
                conv = []

            # Append seed message if missing
            already = any(
                isinstance(m, dict) and m.get("role") == "customer" and m.get("source") == "seed" for m in conv
            )
            if not already:
                conv.append(
                    {
        ...
        day3_ready = (
            mode == "cadence" and last_template_day_sent == 2 and opportunity.get("tk_day3_walkaround_sent") is not True
        )

        log.info(
            "DAY3 DEBUG: opp=%s mode=%r last_template_day_sent=%r day3_ready=%s patti_keys=%s",
            opportunityId,
            mode,
            last_template_day_sent,
            day3_ready,
            list(patti_meta.keys()),
        )
        ...
        if template_day is None:
            ...
            customer = opportunity.get("customer") or {}
            customer_name = (
                (opportunity.get("customer_first_name") or "").strip() or customer.get("firstName") or "there"
            )
            ...
            log.info(
                "FOLLOWUP NAME DEBUG opp=%s customer_name=%r airtable_first=%r cust_first=%r",
                opportunityId,
                customer_name,
...
def _build_email_context(*, opportunity: dict, fresh_opp: dict, subscription_id: str, token: str | None):
    ...
    if not customer_email:
        cust = fresh_opp.get("customer") or fresh_opp.get("customerInfo") or fresh_opp.get("customer_info") or {}
...
def send_first_touch_email(
    *,
    ...
    log.info(
        "send_first_touch_email inputs: opp=%s salesperson=%r customer=%r",
    ...
    if variant not in {VARIANT_LONG, VARIANT_SHORT}:
        log.warning(
            "Unknown AB variant %r for opp=%s; defaulting to %s",
            variant,
            opportunityId,
            VARIANT_LONG,
        )
        variant = VARIANT_LONG
    ...
    # All downstream tracking remains identical — this is a content-only override.
    # ------------------------------------------------------------------
    ...
    if _is_telluride:
        ...
        try:
            ...
            _is_telluride = False  # fall through to normal logic

    # -------------------------------
    # CONTENT PATH 1: Telluride HTML
    # -------------------------------
    if _is_telluride and tel_html:
        cn = (customer_name or "there").strip()
...
def send_thread_reply_now(
    *,
    ...
    ctx = _build_email_context(
        opportunity=opportunity, fresh_opp=fresh_opp, subscription_id=subscription_id, token=token
    )
    ...
    log.info(
        ...
        opportunity.get("customer_email_lower"),
    )
    ...
    if not to_addr:
        log.error(
            ...
            opportunity.get("customer_email"),
            (
                (opportunity.get("customer") or {}).get("email")
                if isinstance(opportunity.get("customer"), dict)
                else None
            ),
            SAFE_MODE,
            test_recipient,
    ...
    log.info(
        "📨 Thread reply composed opp=%s subject=%s short_confirm=%s",
        opportunityId,
        subject,
        bool(created_appt_ok and appt_human),
    )
    log.info(
        "📤 Thread reply send attempt opp=%s to_addr=%s SAFE_MODE=%s test_recipient=%s EMAIL_MODE=%s",
        opportunityId,
        to_addr,
        SAFE_MODE,
        test_recipient,
        os.getenv("EMAIL_MODE", "crm"),
    )
